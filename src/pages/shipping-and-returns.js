...
export default () => {
  const { t, i18n } = useTranslation("shipping_and_returns")

  const productDeliveryMessage =
    i18n.language === "ro" ? "Livrarea Produselor" : "Product Delivery"

  const refoundPolicyMessage =
    i18n.language === "ro" ? "Politica de Retur" : "Refund Policy"
    ...
    <>
      ...
      <StyledArticle deg="-9" paddingTop="0">
        ...
        >
          ...
          {Object.values(
            t("section-1.text", {
              mailTo: mailTo(productDeliveryMessage),
            })
          ).map((item, index) => (
        ...
        >
          ...
          {Object.values(
            t("section-2.text", {
              mailTo: mailTo(refoundPolicyMessage),
            })
          ).map((item, index) => (
            <Box key={index} mx={[0, 2]} my={1}>
              <Typography
